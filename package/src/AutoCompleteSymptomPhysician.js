...
const AutoCompleteSymptom = ({ onAdd }) => {
  ...
  useEffect(() => {
    const fetchMedications = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3001/physician/getSymptoms', {
          method: 'GET',
          headers: {
        ...
        if (Array.isArray(data)) {
          const medNamesArray = data.map((med) => med.Name); // Extract medication names
...
export default AutoCompleteSymptom;
